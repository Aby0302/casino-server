...
export class StakeEngineClient {
  ...
  private cloudInternalToken: string = '';
  ...
  public getReplayData(): any { return this.replayData; }
  ...
  private parseURLParams(): void {
    ...
    this.cloudInternalToken = params.get('cloud_internal') || '';
  ...
  private async fetchWithRetry(url: string, options: RequestInit, retries: number = 3): Promise<Response> {
    ...
    for (let i = 0; i < retries; i++) {
      ...
      try {
        ...
        if (this.cloudInternalToken) headers.set('X-Cloud-Internal', this.cloudInternalToken);
