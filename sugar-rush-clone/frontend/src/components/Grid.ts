...
export class Grid {
  ...
  /**
   * Generate colored circle textures for candy symbols (replaces external PNGs).
   */
  private generateSymbolTextures() {
    const symColors = [0xff8822, 0x2288ff, 0x44cc44, 0xffcc00, 0xff2222, 0x9944ff, 0x00cccc];
    const symKeys = ['candy_0', 'candy_1', 'candy_2', 'candy_3', 'candy_4', 'candy_5', 'candy_6'];
    const size = 64;
    symKeys.forEach(key => { if (this.scene.textures.exists(key)) this.scene.textures.remove(key); });
    symColors.forEach((color, i) => {
      const darker = (c: number) => {
        const r = Math.max(0, ((c >> 16) & 0xff) - 50);
      ...
      g.fillCircle(size / 2 - 8, size / 2 - 8, size / 6);
      g.generateTexture(symKeys[i], size, size);
      g.destroy();
    });
    if (this.scene.textures.exists('scatter')) this.scene.textures.remove('scatter');
    const g = this.scene.add.graphics();
    ...
    g.generateTexture('scatter', 64, 64);
    g.destroy();
  }
